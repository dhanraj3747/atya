...
// Cap container height so the map never dominates the section.
...
export default function Locations() {
  ...
  // On hover, every destination is connected to the active hub, so all destinations
  // stay vibrant. In default state every destination is also vibrant.
  const destOpacity = (_destKey: string) => 1

  // Default: the original "who serves who" network. On hover: active hub fans out
  // to every other city in the list, demonstrating full-India reach from that hub.
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div>
            ...
            >
              ...
              <span className="text-brand-orange text-sm font-body tracking-widest uppercase font-medium">Our Locations</span>
            </motion.div>
            ...
            >
              ...
              <span className="text-brand-orange">across India.</span>
            </motion.h2>
              ...
              {hubs.map((h) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`pointer-events-none absolute -inset-[3px] rounded-2xl bg-[conic-gradient(from_120deg_at_50%_50%,#F26B1F,#F4B400,#9B59F5,#F58A4B,#F26B1F)] blur-[10px] transition-opacity duration-500 ${
                        isActive ? 'opacity-80' : 'opacity-0 group-hover:opacity-50'
                      }`}
                    />
                    {/* Sharper gradient border ring just behind the card */}
                    <div
                      className={`pointer-events-none absolute -inset-px rounded-2xl bg-[conic-gradient(from_120deg_at_50%_50%,#F26B1F,#F4B400,#9B59F5,#F58A4B,#F26B1F)] transition-opacity duration-500 ${
                    ...
                    >
                      ...
                      <p className="font-body text-[11px] text-brand-orange-dark uppercase tracking-widest font-medium mb-3">{h.role}</p>
                      {h.address && (
                      ...
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 mt-3 text-xs font-body font-medium text-brand-orange-dark hover:text-brand-orange transition-colors self-start"
                      >
                        View on Google Maps
          ...
          >
          {/* Layer 1: faint surrounding countries (context) */}
          <img
            src={contextDataUri}
            alt=""
            aria-hidden
          ...
          {/* Layer 2: India — slightly more prominent but still light */}
          <img
            src={indiaDataUri}
            alt=""
            aria-hidden
          ...
          {/* Arcs + pins overlay (matches img aspect exactly via shared aspectRatio) */}
          <svg
          ...
          >
            <defs>
              <linearGradient id="arc-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="#F26B1F" stopOpacity="0" />
                <stop offset="50%" stopColor="#F26B1F" stopOpacity="1" />
                <stop offset="100%" stopColor="#F26B1F" stopOpacity="0" />
              </linearGradient>
            </defs>

            {/* Default network shown when no hub is hovered. On hover, the active
                hub's full fan-out replaces it. Stable keys so arcs that exist in
                both sets (e.g. Bengaluru→Mumbai) don't re-animate. */}
            ...
            {/* Destination pins — softly blinking */}
            {destinations.map((d, i) => (
              ...
              >
                ...
                <circle cx={d.x} cy={d.y} r={0.35} fill="#F26B1F" opacity={0.3}>
                ...
                {/* Core dot — blinks */}
                <circle cx={d.x} cy={d.y} r={0.35} fill="#F26B1F">
                  <animate attributeName="opacity" values="1;0.55;1" dur="1.8s" begin={`${i * 0.25}s`} repeatCount="indefinite" />
                </circle>
              </g>
            ))}

            {/* Hub pins — double-ring radar ping. When active, scale up via larger core. */}
            {hubs.map((h, i) => {
              const isActive = activeHub === h.key
              return (
                <g
                ...
                >
                  ...
                  <circle cx={h.x} cy={h.y} r={0.7} fill="none" stroke="#F26B1F" strokeWidth={isActive ? 0.18 : 0.12} opacity={0.6}>
                  ...
                  <circle cx={h.x} cy={h.y} r={0.7} fill="none" stroke="#F26B1F" strokeWidth={isActive ? 0.22 : 0.15} opacity={0.8}>
                  ...
                  {/* Core dot — solid + subtle breathing; grows when active */}
                  <circle
                    cx={h.x}
                    cy={h.y}
                  ...
                  {/* Small white inner dot for "active" look */}
